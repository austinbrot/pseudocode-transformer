    ...
    def __post_init__(self):
        if self.train_file is None and self.validation_file is None:
            ...
            if self.train_file is not None:
                ...
                assert extension in [
                    "json"], "`train_file` should be a json file."
            if self.validation_file is not None:
                extension = self.validation_file.split(".")[-1]
                assert extension in [
                    "json"], "`validation_file` should be a json file."
        if not self.task.startswith("translation"):
            raise ValueError(
                "translation or translation_{xx}_to_{yy}."
            )
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        revision=model_args.model_revision,
        use_auth_token=True if model_args.use_auth_token else None,
        additional_special_tokens=('[PRE]', '[ENDL]', '[PRED]')
    )
    ...
    # them all).
    source_lang, target_lang, text_column, summary_column = 'en', 'c++', None, None
    ...
    if training_args.do_predict:
        ...
        print(data_args.num_return_sequences)
        total_batches = ceil(len(test_dataset)//batch_size)
        for i in tqdm(range(total_batches)):
            start_ind = batch_size*i
            end_ind = min(len(test_dataset),batch_size*(i+1))
            batch = test_dataset[start_ind:end_ind]
            #if i%500==0:
                #print('{}%'.format(round(100*i/len(test_dataset),2)))
            gen = model.generate(input_ids=torch.tensor(batch['input_ids'],dtype=torch.int64,device='cuda:0'),\
                    attention_mask=torch.tensor(batch['attention_mask'],dtype=torch.int64,device='cuda:0'),\
                    max_length=data_args.val_max_target_length,num_beams=data_args.num_beams,num_return_sequences=data_args.num_return_sequences)
            top_preds = tokenizer.batch_decode(gen,skip_special_tokens=True, clean_up_tokenization_spaces=True)
            for j in range(end_ind-start_ind):
                test_preds.append('\t'.join([pred.strip() for pred in top_preds[j*data_args.num_return_sequences:(j+1)*data_args.num_return_sequences]]))
